...
int main() {
...

	{
		...
		ASSERT_EQUAL_MSG(list_task, list_std, "list::erase")
			/*   task::list<size_t>::iterator it = list_task.begin();
			  std::list<size_t>::iterator it2 = list_std.begin();

			  for (; it != list_task.end(); ++it)
				  std::cout << *(it) << ' ';
			  std::cout << std::endl;

			  for (it2 = list_std.begin(); it2 != list_std.end(); it2++)
				  std::cout << *(it2) << ' ';
			  std::cout << std::endl;    */
	}
...

	{
		...
		ASSERT_EQUAL_MSG(list_task2, list_std2, "list::swap")
		{
			...
			std::cout << "std\n";
			for (auto it = list_std.begin(); it != list_std.end(); it++)
				std::cout << *(it) << ' ';
			std::cout << std::endl;*/
